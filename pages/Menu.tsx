...
const MenuItemCard = React.memo<{
  ...
  ({ item, qty, onItemClick, onImageClick, onAddToCart, onUpdateQuantity }) => {
    return (
      <div
...
export const Menu: React.FC = () => {
  ...
  // Memoize callbacks to prevent recreating functions on every render
  ...
  // Reset modal state when opening an item (removing duplicate)
  const openItemModal = handleItemClick;
  ...
  // Memoize cart quantity map for O(1) lookups instead of O(n) for each item
      ...
      <div className="min-h-screen bg-neutral-900 p-4 space-y-4">
        ...
        <div className="flex space-x-4 overflow-hidden mb-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-8 w-24 bg-neutral-800 rounded-full animate-pulse flex-shrink-0"
            ></div>
          ))}
        </div>
        {[1, 2, 3, 4, 5].map((i) => (
              ...
              <div className="flex gap-4">
                ...
                <div className="flex items-center bg-neutral-800 rounded-xl border border-neutral-700 h-14 px-2">
                  <button
                    ...
                    className="w-10 h-full text-gray-400 hover:text-white text-xl"
                  >
                    -
                  </button>
                  ...
                  <button
                    ...
                    className="w-10 h-full text-brand-gold text-xl"
                  >
                    +
                  </button>
                </div>
